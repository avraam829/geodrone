...
const Map = ({ mapStyle }) => {
  ...
  useEffect(() => {
    ...
    if (mapRef.current) {
      ...
      mapRef.current.on('style.load', () => {
        ...
        // Initialize Threebox and check if it initializes successfully
        ...
        if (window.tb) {
          ...
          console.error("Failed to initialize Threebox");
        }
  ...
  // Вынесли createSphere за пределы useEffect
  const createSphere = useCallback((lon, lat, height) => {
    let origin = [lon, lat, height];
    console.log("Creating sphere at coordinates:", origin);
    return window.tb.sphere({ color: 'red', material: 'MeshToonMaterial', radius: 500 }) // Увеличили радиус до 500
      .setCoords(origin);
  }, []);
